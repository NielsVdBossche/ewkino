...
NanoReader::LeptonReader::LeptonReader(NanoReader& nano, std::string lepton) :
    nanoReader(nano)
{
    leptonType = new std::string(lepton);
    //std::cout << leptonType << std::endl;
}


void NanoReader::LeptonReader::InitTree(TTree* _currentTreePtr) {
...
void NanoReader::LeptonReader::setOutputTree(TTree* tree) {
...
NanoReader::LightLeptonReader::LightLeptonReader(NanoReader& nano, std::string lepton) :
    LeptonReader(nano, lepton)
{
    // std::cout << *leptonType << std::endl;
}


void NanoReader::LightLeptonReader::InitTree(TTree* _currentTreePtr) {
    // std::cout << *leptonType << std::endl;
    LeptonReader::InitTree(_currentTreePtr);
...
void NanoReader::LightLeptonReader::setOutputTree(TTree* tree) {
    LeptonReader::setOutputTree(tree);
    ...
    tree->Branch((*leptonType+"_jetBTagDeepFlavor").c_str(),  _Lepton_jetBTagDJ,        (*leptonType+"_jetBTagDeepFlavor[n"+*leptonType+"]/F").c_str());

    if (GetNanoReader().containsGeneratorInfo()) {
        tree->Branch((*leptonType+"_isPrompt").c_str(),             _Lepton_isPrompt,             (*leptonType+"_isPrompt[n"+*leptonType+"]/O").c_str());
        tree->Branch((*leptonType+"_isChargeFlip").c_str(),         _Lepton_isChargeFlip,         (*leptonType+"_isChargeFlip[n"+*leptonType+"]/O").c_str());
        tree->Branch((*leptonType+"_matchPdgId").c_str(),           _Lepton_matchPdgId,           (*leptonType+"_matchPdgId[n"+*leptonType+"]/I").c_str());
        tree->Branch((*leptonType+"_motherPdgId").c_str(),          _Lepton_motherPdgId,          (*leptonType+"_motherPdgId[n"+*leptonType+"]/I").c_str());
        tree->Branch((*leptonType+"_provenanceConversion").c_str(), _Lepton_provenanceConversion, (*leptonType+"_provenanceConversion[n"+*leptonType+"]/O").c_str());
    }
}
