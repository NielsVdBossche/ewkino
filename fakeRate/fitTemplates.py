...
def loadallhistograms(histfile,mustcontain=[]):
    ...
    # if mustcontain is not empty, an extra selection is done:
    # histogram names are required to contain all strings in it, else they are ignored.
    ...
    for key in keylist:
        ...
        if len(mustcontain)>0:
            for tag in mustcontain:
                if not str(tag) in hist.GetName(): keep = False; break
...
def tgraphtohist( graph ):
    ### convert a TGraph object to a TH1 object
