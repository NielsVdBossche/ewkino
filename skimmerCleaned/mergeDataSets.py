...
if __name__=='__main__':
    ...
    cmd = '{} $TMPDIR/{}'.format(exe,targetfile.split('/')[-1])
    for f in inputfiles: cmd += ' {}'.format(f)

    cmd += "\n\n"
    cmd += "cp $TMPDIR/*.root {}/".format(os.path.dirname(targetfile))
    # submit the command
    ct.submitCommandAsCondorJob( 'cjob_mergeDataSets', cmd , cmssw_version="CMSSW_13_3_3")
